...
export class WSMailbox extends EventEmitter<MailboxEvent> implements Mailbox {
  protected curId = 0
  ...
  connect(tracer?: Tracer): Promise<void> {
    ...
    if (this.connected) {
      ...
      p.reject(new Error('WSMailbox has already connected.'))
      return p.promise
    }

    const socket = this.socket = new WebSocketClient(`ws://${this.host}:${this.port}`, {
    ...
    socket.onerror = (event: any) => {
      ...
      if (event && event.error) this.logger.error(event.error)
      this.close()
    }
    socket.onclose = () => {
      this.logger.info('[regax-rpc] rpc socket %s is disconnect from remote server %s', this.clientId, this.id)
  ...
  send(msg: any, tracer?: Tracer): Promise<void> {
    ...
    const timer = setTimeout(() => {
      ...
      const eMsg = util.format('rpc(%s.service.%s) callback timeout %d, remote server host: %s, port: %s',
        this.clientId, msg && msg.service, this.invokeTimeout, this.host, this.port)
    ...
    if (this.lastPing > 0) {
      if (this.lastPong < this.lastPing) {
        if (now - this.lastPing > KEEP_ALIVE_TIMEOUT) {
          this.logger.error('[regax-rpc] websocket rpc client %s checkKeepAlive timeout from remote server %s for %d lastPing: %s lastPong: %s',
